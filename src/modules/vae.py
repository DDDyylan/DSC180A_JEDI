...
class VAE(nn.Module):
    ...
    def forward(self, inputs, labels, std=False, return_z=False, return_mu=False):
        ...
        if self.args.fb_mode == 0:
            ...
            outputs = self.decoder(input_ids=labels, past=latent_z, labels=labels, label_ignore=self.pad_token_id)
            loss_rec = outputs[0]  # model outputs are always tuple in pytorch-transformers (see doc)
        elif self.args.fb_mode == 4:  # For diffusion autoencoder 
            # Connect hidden feature to the latent space
            mu, logvar = self.encoder.linear(pooled_hidden_fea).chunk(2, -1)
            latent_z = self.reparameterize(mu, logvar, nsamples=1)
            latent_z = latent_z.squeeze(1)
            ...
            # outputs = self.decoder(input_ids=labels, past=latent_z, labels=labels, label_ignore=self.pad_token_id)
            # loss_rec = outputs[0]  # model outputs are always tuple in pytorch-transformers (see doc)
            return 0, 0, 0, latent_z, mu
        elif self.args.fb_mode == 5:
            mu, logvar = self.encoder.linear(pooled_hidden_fea).chunk(2, -1)
        ...
        if return_z:
            return loss_rec, loss_kl, loss, latent_z
        elif return_mu:
            return loss_rec, loss_kl, loss, latent_z, mu
        elif std:
...
class DenseEmbedderForAdv(DenseEmbedder):
    def __init__(self, input_dim, up_dim, depth=4, num_classes=10, given_dims=None, norm=None):
        super().__init__(input_dim, up_dim, depth, num_classes, given_dims, norm)
...
class DenseEmbedderTime(nn.Module):
    def __init__(self, latent_dim, num_classes=2, T=2000):
        super().__init__()
        # time embedding
        self.n_T = T
        ...
        use_norm = True
        num_layers = 5
        num_hid_channels = 512 # latent_dim * 4
        ...
        for i in range(num_layers):
            if i == 0:
                ...
                norm = False
                cond = False
                a, b = num_hid_channels, num_classes
                dropout = dropout
            else:
                act = self.activation
                norm = use_norm
        ...
        for k, v in ddpm_schedules(1e-4, 0.02, self.n_T).items():
    ...
    def forward(self, x):
        ...
        return self.train_step(x_t,_ts)
...
class CCF(nn.Module):
    ...
    def get_cond_energy_time(self, z, y_, t_k):
        energy_outs = []
        # for i, cls in enumerate(self.f):
        for i in range(y_.shape[1]):
            cls = self.f[i]
