...
from .vae import DenseEmbedder,DenseEmbedderForAdv
import pdb
import numpy as np
import logging
...
class DAAE(nn.Module):
    ...
    def __init__(self, encoder, decoder,  tokenizer_encoder, tokenizer_decoder, args): # 
        ...
        self.D = DenseEmbedderForAdv(args.latent_size, 4, 4, 1)
        self.args = args
...
#         pdb.set_trace() 
    ...
    def forward(self, inputs, labels):
        if self.args.noise:
            inputs = self.word_drop(inputs,0.15)
...
#         ipdb.set_trace()
    ...
    def loss_adv(self, z):
        ...
        zeros = torch.zeros((zn.shape[0],1),dtype=zn.dtype, device=zn.device) 
        # zeros = torch.zeros_like(zn[:,[0]])
        # ones = torch.ones_like(zn[:,[0]])
        ones = torch.ones((zn.shape[0],1),dtype=zn.dtype, device=zn.device)
        ...
        # loss_d = self.BCEWithLogitsLoss(self.D(z.detach()),zeros)+ \
        #     self.BCEWithLogitsLoss(self.D(zn), ones)
        # loss_g = self.BCEWithLogitsLoss(self.D(z), ones) # 错误的entropy
